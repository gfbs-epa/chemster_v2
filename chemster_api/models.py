...
# SQLAlchemy models for tables in existing SQLite database
class User(db.Model):
    __tablename__ = 'users'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    username = db.Column(db.String(255, collation='NOCASE'), nullable=False, unique=True, index=True)
...
class Collection(db.Model):
    ...
    super_id = db.Column(db.Integer, db.ForeignKey('collections.id'), index=True)
...
class CollectionChemical(db.Model):
    ...
    collection_id = db.Column(db.Integer, db.ForeignKey('collections.id'), nullable=False, index=True)
    chemical_dtxsid = db.Column(db.String(16, collation='NOCASE'), db.ForeignKey('chemicals.dtxsid'), nullable=False, index=True)
